 ...
 *   SOCLE_INLINE         (optional) "false" pour publier les %%=ContentBlockByKey()=%%
 *                        tels quels au lieu d'inliner le socle SSJS dans la page
 */

const { inlineSocleBlocks } = require('./socle-inliner');
const { poserEcoleSurLaPage } = require('./ecole-page');
...
/**
 * Dossier de destination des pages publiées.
 *
 * Par défaut : `<SFMC_CATEGORY_NAME>/pages/<schoolId>`, créé au besoin.
 *
 * Renseigner SFMC_PAGES_CATEGORY_ID (id relevé dans Content Builder) force un
 * dossier UNIQUE et court-circuite la création de `pages/<schoolId>`. Sert à
 * pointer un dossier de recette partagé par toutes les écoles — `test_forms`
 * dans la BU EDH Recette. L'id est propre à une BU : le changer sans changer
 * SFMC_ACCOUNT_ID publierait dans un dossier inexistant.
 */
const SFMC_PAGES_CATEGORY_ID = process.env.SFMC_PAGES_CATEGORY_ID
    ? parseInt(process.env.SFMC_PAGES_CATEGORY_ID, 10)
    : null;
...
/**
 * INTERRUPTEUR GLOBAL SFMC — desactive temporairement TOUTE l'integration
 * (login OAuth, publication de pages, upload d'images, dependublication) en
 * local ET en prod, tant que la lecture Salesforce n'est pas finalisee.
 *
 * Desactive par defaut : la sync SFMC ne part plus, aucune erreur ne bloque la
 * sauvegarde/publication (tous les appelants passent par isSfmcConfigured() et
 * ignorent proprement quand c'est false).
 *
 * Pour REACTIVER quand la lecture sera prete : poser SFMC_SYNC_ENABLED=true
 * dans l'environnement (local .env et/ou variables de prod). Rien d'autre a
 * changer dans le code.
 */
const SFMC_SYNC_ENABLED = String(process.env.SFMC_SYNC_ENABLED || '').toLowerCase() === 'true';
...
function isSfmcConfigured() {
    // Interrupteur global : si la sync est desactivee, on se comporte comme si
    // SFMC n'etait pas configure -> tous les appels sont ignores sans erreur.
    if (!SFMC_SYNC_ENABLED) return false;
    return isSfmcCredentialsConfigured();
}

/**
 * Identifiants SFMC presents, INDEPENDAMMENT de l'interrupteur de synchro.
 *
 * L'interrupteur SFMC_SYNC_ENABLED ne vise que la SYNCHRO SORTANTE (publication
 * de pages, upload d'assets, depublication). Le store campus, lui, est une
 * simple Data Extension lue/ecrite par l'editeur : le couper faisait repondre
 * 500 a /api/campuses, donc « 0 campus en base » dans la modale alors que la
 * DE etait bien remplie. La gestion des campus n'a donc besoin QUE des
 * identifiants, pas de l'interrupteur.
 */
function isSfmcCredentialsConfigured() {
...
async function getAccessToken() {
    // Identifiants seulement : le jeton sert aussi au store campus, qui reste
    // disponible quand la synchro sortante est coupee. Les fonctions de
    // publication (assets, DE de formulaires…) gardent leur propre
    // assertConfigured() et restent donc bien bloquees par l'interrupteur.
    assertCredentials();
...
async function buildAssetPayload({ projectName, fullHtml }) {
    // Le socle SSJS est injecté ICI, au dernier moment : la page envoyée à SFMC
    // est autonome (aucun Content Block à créer dans Content Builder), tandis
    // que le HTML stocké en base garde la forme courte %%=ContentBlockByKey()=%%.
    // Désactivable avec SOCLE_INLINE=false pour repasser aux Content Blocks.
    let htmlFinal = fullHtml;

    /* L'ecole de la page, AVANT l'inlining : tant que le socle est encore un
       %%=ContentBlockByKey()=%%, on sait exactement ou inserer le prelude.
       Sans effet sur une page construite apres le correctif du builder, qui
       porte deja l'ecole — voir lib/ecole-page.js. */
    const ecolePage = getSchoolIdFromProjectName(projectName);
    const pose = poserEcoleSurLaPage(htmlFinal, ecolePage);
    htmlFinal = pose.html;
    if (pose.prelude || pose.marque) {
        console.log(`🏫 SFMC: ecole « ${ecolePage} » posee sur la page` +
                    `${pose.prelude ? ' (prelude @LPB_*)' : ''}` +
                    `${pose.marque ? ' (champ Marque)' : ''}` +
                    ' — page anterieure au correctif du builder');
    }

    if (process.env.SOCLE_INLINE !== 'false') {
        /* ⚠ htmlFinal, PAS fullHtml. Repartir de l'original jetait en silence
           le prelude que poserEcoleSurLaPage venait d'ajouter : les pages
           publiees n'ont jamais porte @LPB_ECOLE, ni le type de formulaire, ni
           celui d'evenement. Le journal annoncait pourtant « ecole posee ».
           Constate le 30/08 sur une page JPO sans aucune date. */
        const r = inlineSocleBlocks(htmlFinal);
        if (r.inline) {
            htmlFinal = r.html;
            console.log(`🧩 SFMC: socle inliné (${r.blocs.length} bloc(s) : ${r.blocs.join(', ')}` +
                        `${r.dedoublonnes ? `, ${r.dedoublonnes} dédoublonné(s)` : ''}, +${Math.round(r.octets / 1024)} Ko)`);
        }
    }
...
async function syncProjectToSfmc({ projectName, fullHtml }) {
    ...
    // 1..3. Dossier de destination.
    // Un id explicite gagne : on publie DIRECTEMENT dedans, sans créer
    // `pages/<schoolId>` — sinon un dossier de recette se retrouverait
    // sous-découpé par école, ce qui n'est pas ce qu'on veut.
    let schoolFolderId;
    let folderLabel;

    if (SFMC_PAGES_CATEGORY_ID) {
        schoolFolderId = SFMC_PAGES_CATEGORY_ID;
        folderLabel = `#${SFMC_PAGES_CATEGORY_ID} (SFMC_PAGES_CATEGORY_ID)`;
    } else {
        ...
        schoolFolderId = await ensureFolder(schoolId, pagesFolderId);
        folderLabel = `${SFMC_CATEGORY_NAME}/pages/${schoolId}`;
    }
...
/** Appel SOAP générique contre l'API partner SFMC (auth via fueloauth).
    SOAP ne sert QUE au store campus (DE) → identifiants suffisants. */
...
/* ==========================================================================
   DICTIONNAIRE DE TRADUCTION DES VALEURS DE PICKLIST
   ==========================================================================
   La DE `LPB_Dico_Traductions` porte la correspondance FR -> EN des libelles
   de picklist Salesforce. Elle ne sert QU A L AFFICHAGE : la valeur postee par
   le formulaire et ecrite dans le CRM reste toujours l originale.

   Alimentation : le bloc CloudPage `LPB_TST_Dico_Sync` insere les libelles
   absents, l anglais vide. Traduction : `scripts/traduire-dico.js`, qui appelle
   ces deux fonctions. Correction manuelle : directement dans la DE.

   Meme transport SOAP que les campus : REST ne sait pas ecrire de lignes de DE
   sur cette org. La pagination, elle, est implementee ici — `MoreDataAvailable`
   tronquerait silencieusement un dictionnaire de plusieurs centaines de lignes.
   ========================================================================== */
const DICO_DE_KEY = 'LPB_Dico_Traductions';
const DICO_FIELDS = ['Cle', 'Fr', 'En', 'Champ', 'Origine', 'Actif'];

/** Parse les <Results> d une reponse Retrieve, sans exiger de colonne `id`. */
function parseDicoRows(text) {
    const rows = [];
    const blocks = text.match(/<Results\b[\s\S]*?<\/Results>/g) || [];
    for (const block of blocks) {
        const o = {};
        ...
        if (o.Cle) rows.push(o);
    }
    return rows;
}

/** Lit TOUTES les lignes du dictionnaire, pagination comprise. */
async function lireDicoTraductions() {
    const propsXml = DICO_FIELDS.map(p => `<Properties>${p}</Properties>`).join('');
    let inner = `<RetrieveRequestMsg xmlns="http://exacttarget.com/wsdl/partnerAPI"><RetrieveRequest>`
        + `<ObjectType>DataExtensionObject[${soapEsc(DICO_DE_KEY)}]</ObjectType>${propsXml}`
        + `</RetrieveRequest></RetrieveRequestMsg>`;
    ...
    for (let page = 0; page < 40; page++) {
        const res = await soapRequest('Retrieve', inner);
        if (res.overall !== 'OK' && res.overall !== 'MoreDataAvailable') {
            if (!/<Results\b/.test(res.text)) return toutes;   // DE vide
            throw soapError('Lecture du dictionnaire echouee', res);
        }
        toutes.push(...parseDicoRows(res.text));
        if (res.overall !== 'MoreDataAvailable') return toutes;

        const suite = (res.text.match(/<RequestID>([^<]*)<\/RequestID>/i) || [])[1];
        if (!suite) return toutes;
        inner = `<RetrieveRequestMsg xmlns="http://exacttarget.com/wsdl/partnerAPI"><RetrieveRequest>`
            + `<ObjectType>DataExtensionObject[${soapEsc(DICO_DE_KEY)}]</ObjectType>${propsXml}`
            + `<ContinueRequest>${soapEsc(suite)}</ContinueRequest>`
            + `</RetrieveRequest></RetrieveRequestMsg>`;
    }
    return toutes;
}
...
/**
 * Upsert de lignes du dictionnaire, par lots.
 *
 * @param {Array<{Cle:string, En:string}>} lignes
 */
async function upsertDicoTraductions(lignes) {
    ...
    for (let i = 0; i < lignes.length; i += TAILLE_LOT) {
        ...
        const objectsXml = lot.map(l => {
            const props = Object.keys(l)
                .map(k => `<Property><Name>${soapEsc(k)}</Name><Value>${soapEsc(l[k])}</Value></Property>`)
                .join('');
            return `<Objects xsi:type="DataExtensionObject">`
                + `<CustomerKey>${soapEsc(DICO_DE_KEY)}</CustomerKey>`
                + `<Properties>${props}</Properties></Objects>`;
        }).join('');

        const inner = `<UpdateRequest xmlns="http://exacttarget.com/wsdl/partnerAPI">`
            + `<Options><SaveOptions><SaveOption>`
            + `<PropertyName>*</PropertyName><SaveAction>UpdateAdd</SaveAction>`
            + `</SaveOption></SaveOptions></Options>`
            + `${objectsXml}</UpdateRequest>`;

        const res = await soapRequest('Update', inner);
        if (res.overall !== 'OK') throw soapError('Ecriture du dictionnaire echouee', res);
        ecrites += lot.length;
    }
    return ecrites;
}
...
module.exports = {
    /* Exportee pour les tests : c'est ELLE qui enchaine le prelude puis
       l'inlining, et cet enchainement s'est deja perdu une fois. */
    buildAssetPayload,
    isSfmcConfigured,
    isSfmcCredentialsConfigured,
    syncProjectToSfmc,
    ...
    assetNameFor,
    lireDicoTraductions,
    upsertDicoTraductions,
    /* Primitives SOAP, pour les scripts qui ecrivent dans une DE (config API). */
    soapRequest,
    soapEsc
};
