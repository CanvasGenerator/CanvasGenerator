...
/* ═══════════════════════════════════════════════════════════════
   ...
   Remplace l'ancien stockage Supabase. Même modèle : id, name, slug,
   image_url, address, link. CRUD via REST (list/upsert) + SOAP (delete).
═══════════════════════════════════════════════════════════════ */
...
/* Champs de la DE campus (SOAP). id = clé primaire. */
const CAMPUS_FIELDS = [
    { name: 'id',        length: 100,  pk: true,  required: true },
...
async function ensureCampusDE() {
    ...
    if (!exists) {
        ...
        // 3. Seed par défaut (uniquement si vide)
        try {
            const rows = await listCampuses(true);
            if (!rows.length) {
                console.log(`🌱 SFMC: seed de ${DEFAULT_CAMPUSES.length} campus par défaut...`);
                await upsertCampuses(DEFAULT_CAMPUSES);
            }
        } catch (e) {
            console.warn(`⚠️ SFMC campus seed: ${e.message}`);
        }
    }
...
/** Liste tous les campus depuis la DE (triés par nom). */
async function listCampuses(skipEnsure = false) {
    ...
    const rows = parseSoapRows(res.text).map(r => ({
        id: r.id,
        name: r.name || '',
        slug: r.slug || r.id,
...
/** Upsert (insert/update) d'une liste de campus par clé primaire `id`. */
async function upsertCampuses(list) {
    await ensureCampusDE();
    const rows = (list || []).filter(c => c && c.id);
    if (!rows.length) return { rows: 0 };

    const objectsXml = rows.map(c => {
        const vals = { id: String(c.id), name: c.name || '', slug: c.slug || c.id || '',
...
/** Upsert d'un seul campus → renvoie l'objet campus normalisé. */
...
/** Supprime un campus par `id` (SOAP DataExtensionObject Delete). */
async function deleteCampus(id) {
    ...
    const inner = `<DeleteRequest xmlns="http://exacttarget.com/wsdl/partnerAPI">`
        ...
        + `<Keys><Key><Name>id</Name><Value>${soapEsc(id)}</Value></Key></Keys></Objects></DeleteRequest>`;
