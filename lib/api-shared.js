...
/**
 * Nettoie le libellé d'une page déclinée depuis un template maître.
 *
 * Le nom proposé par la modale est le titre DU MASTER, qui contient presque
 * toujours son marqueur (« MASTER TEMPLATE - CANDIDATURE »). Sans nettoyage, la
 * page école héritait de ce marqueur (« ICART – MASTER TEMPLATE - CANDIDATURE »)
 * et une seconde déclinaison au nom nettoyé créait une page SŒUR au lieu de
 * mettre à jour la première → doublons dans le dashboard.
 *
 * Le résultat est déterministe : décliner deux fois le même master produit le
 * même nom, donc un upsert et non un doublon.
 */
function normalizeDeclinedTitle(rawName) {
    const base = String(rawName || '')
        // préfixe technique éventuel (school-master__, school-icart__…)
        .replace(/^school-[a-z0-9-]+_+/i, '')
        // suffixe de langue (__FR, __EN)
        .replace(/__[a-z]{2}$/i, '')
        .trim();

    // Marqueur de template maître en tête du libellé. Le `\b` est indispensable :
    // sans lui, « Mastere Specialise » et « MASTERCLASS Design » étaient tronqués.
    const stripped = base
        .replace(/^master(?:\s+template)?\b\s*(?:[-–—:]\s*)?/i, '')
        .trim();

    // Un libellé qui n'était QUE le marqueur (« MASTER ») ne doit pas devenir vide :
    // on retomberait sur un nom de projet « school-icart____FR ».
