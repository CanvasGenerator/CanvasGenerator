...
function buildStoredHtml({ projectName, html = '', css = '', properties = {} }) {
    ...
    const bodyContent = isFullHtmlDocument(html) ? extractBodyContent(html) : html;
    ...
    return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${escapeHtml(title)}</title>${seoTags}<style>${css}</style></head><body>${bodyContent}</body></html>`;
}
