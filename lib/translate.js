...
// Bascule les logos "avec baseline" (header/footer) vers leur variante anglaise
// baseline-<x>-en.png. Ne touche PAS aux "nobaseline" (pas de tagline à traduire).
// Idempotent. Utilisé quand la langue cible est l'anglais.
function swapBaselineLogosToEn(html) {
    return String(html || '').replace(/(\/baseline-[a-z]+)(?:-en)?\.png/gi, '$1-en.png');
}
function isEnglishTarget(lang) {
    return /^(en|eng|english|anglais)$/i.test(String(lang || '').trim());
}
 ...
 */
/* Un echec passager de Gemini — surcharge du modele, 429, 5xx — faisait
   echouer TOUTE la traduction : un seul lot en erreur et l appelant perdait les
   lots deja traduits. Trois tentatives avec attente croissante suffisent a
   absorber les pics, constates le 28/08/2026 sur un dictionnaire de 415
   libelles. Les erreurs de FORME (JSON illisible, longueur incoherente) ne sont
   PAS reessayees : elles ne passeront pas mieux la fois suivante. */
const RETRYABLE = /high demand|overload|unavailable|try again|rate limit|quota|timeout|429|50\d/i;

function estPassagere(e) {
    return RETRYABLE.test(String((e && e.message) || ''));
}

const dodo = (ms) => new Promise(r => setTimeout(r, ms));

async function translateBatch(strings, targetLang, apiKey, fetchImpl, tentatives = 3) {
    let derniere;
    for (let essai = 1; essai <= tentatives; essai++) {
        try {
            return await appelerGemini(strings, targetLang, apiKey, fetchImpl);
        } catch (e) {
            derniere = e;
            if (essai === tentatives || !estPassagere(e)) throw e;
            await dodo(essai * 4000);
        }
    }
    throw derniere;
}

async function appelerGemini(strings, targetLang, apiKey, fetchImpl) {
...
async function translateHtml(html, targetLang, apiKey, fetchImpl = globalThis.fetch) {
    ...
    let out = $.html();
    // Anglais : basculer aussi les logos header/footer vers leur version -en.
    if (isEnglishTarget(targetLang)) out = swapBaselineLogosToEn(out);
    return out;
}

/* `translateStrings` est exportee depuis le 28/08/2026 : elle traduit un
   TABLEAU DE CHAINES, ce dont le dictionnaire de picklists a besoin
   (scripts/traduire-dico.js). Jusque-la, seule `translateHtml` etait publique,
   et le front devait emballer ses chaines dans des <div id="tN"> pour
   l'atteindre (js/app.js, translateStringList). Ce contournement reste en
   place cote front, mais rien de nouveau ne devrait le reprendre. */
module.exports = { translateHtml, translateStrings, collectTargets, isFullDocument };
