...
function combineBilingualHtml(shellFullHtml, bodiesByLang, currentLang) {
    ...
    const script = '<script>(function(){try{'
        ...
        // ⚠️ Un <script> inséré via innerHTML n'est JAMAIS exécuté par le navigateur.
        // Sans ce ré-amorçage, les comportements des blocs (onglets, +/- FAQ,
        // carrousels — portés par le script exporté par GrapesJS) meurent dès la
        // 1re bascule de langue. On recrée chaque script pour le réexécuter.
        + 'p.innerHTML=D[n];'
        + 'var ss=p.querySelectorAll("script");'
        + 'for(var k=0;k<ss.length;k++){var o=ss[k],s=document.createElement("script");'
        + 'if(o.type){s.type=o.type;}s.text=o.textContent||"";o.parentNode.replaceChild(s,o);}'
        + 'L=n;paint();'
        + '},true);'
        + '}catch(err){}})();</script>';
