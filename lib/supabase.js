...
async function supabaseRequest(method, endpoint, body = null, extraHeaders = {}) {
    ...
    if (!response.ok) {
        ...
        const detail = payload ? ` — ${payload.message || payload.hint || payload.details || JSON.stringify(payload)}` : '';
        const err = new Error(`Supabase ${method} ${endpoint} failed (HTTP ${response.status})${detail}`);
