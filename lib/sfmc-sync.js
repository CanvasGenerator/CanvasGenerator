...
const { syncLegacyProjectToContent, isMissingContentSchemaError, getBilingualHtmlForProject } = require('../api/content');

async function enqueueOrProcessInline({ projectName, language, fullHtml, css, projectData, properties, source }) {
    try {
        await supabaseRequest('POST', '/integration_jobs', {
            target:       'sfmc',
            action:       'sync_project',
            status:       'pending',
            payload:      { projectName, language, html: fullHtml },
        ...
        try {
            // Page bilingue auto-portée si ≥2 langues (même logique que le cron worker).
            let sourceHtml = fullHtml;
            try { sourceHtml = (await getBilingualHtmlForProject(projectName))?.html || fullHtml; }
            catch (biErr) { /* pas de variantes / schéma absent → HTML simple */ }
            let cleaned = cleanHtmlForSfmc(sourceHtml);
            if (isSfmcConfigured()) {
            ...
            // Brouillon : on prépare html_sfmc + images, mais on NE publie PAS la page.
            // La publication de l'asset webpage se fait manuellement via /api/publish-sfmc.
