...
console.log('\n5. Périmètre protégé — le rendu public ne doit JAMAIS être verrouillé');
{
    ...
    const OPEN = ['/login', '/handlelogin', '/logout', '/api/cron',
                  ...
                  // Diagnostic + APIs consommées par les pages publiées.
                  '/api/auth-status',
                  '/api/campuses', '/api/campuses?school=efap', '/api/campuses/12',
                  '/api/faq/render'];
    ...
    it('/api/campuses servie sans session (carrousels des pages en ligne)', () => {
        const res = fakeRes();
        assert.strictEqual(mod.requireSfmcAuth(reqWith(null), res, '/api/campuses'), true);
        assert.strictEqual(res.statusCode, null);
    });
    restore();
}

console.log('\n5b. Diagnostic /api/auth-status');
{
    const { mod, restore } = load(FULL);
    it('signale les variables manquantes et le redirect_uri exact', () => {
        const { mod: m2, restore: r2 } = load({ ...FULL, SFMC_AUTH_CLIENT_SECRET: '' });
        const st = m2.authStatus(reqWith(null));
        assert.strictEqual(st.configComplete, false);
        assert.ok(st.missingVars.includes('SFMC_AUTH_CLIENT_SECRET'));
        assert.strictEqual(st.redirectUri, 'https://app.example.com/handlelogin');
        r2();
    });
    it('ne divulgue aucun secret', () => {
        const dump = JSON.stringify(mod.authStatus(reqWith(null)));
        assert.ok(!dump.includes(FULL.SESSION_SECRET), 'SESSION_SECRET divulgue');
        assert.ok(!dump.includes('secret'), 'client_secret divulgue');
    });
    it('expose l\'état lu par js/auth-guard.js', () => {
        const st = mod.authStatus(reqWith(null));
        assert.strictEqual(st.authActive, true);
        assert.strictEqual(st.sessionPresent, false);
        const v = mod.serializeSession(
            { user: { id: 'u1' }, expires: Math.floor(Date.now() / 1000) + 600 }, FULL.SESSION_SECRET);
        assert.strictEqual(
            mod.authStatus(reqWith(`sfmc_session=${encodeURIComponent(v)}`)).sessionPresent, true);
    });
    restore();
}
