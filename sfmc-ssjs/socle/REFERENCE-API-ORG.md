...
## 2 septembre 2026 - les trois champs du retour client sur le consentement

Retour client : sur les 4 `ContactPointConsent` d'une soumission, trois champs
sortent vides — Marque, Statut RGPD, Dernière date d'engagement marketing.

Les 4 records ont été identifiés dans `LPB_Log_Soumissions` : soumission du
**02/09 04h25**, étape « 60 - consentement », un record par canal (Email,
Phone, SMS, WhatsApp). Ils viennent donc bien du handler déployé la veille.

### Deux champs portent le MEME libellé « Dernière date d'engagement marketing »

Relevé dans l'Object Manager de l'org, et compté sur les 4227 consentements de
l'extrait de recette :

| Nom d'API | Type | Rempli |
|---|---|---|
| `LastMarketingEngagementDate__c` | Date/heure | 4 / 4227 |
| `Last_Marketing_Engagement_Date__c` | Date | **4143 / 4227** |

Le retour client nomme le **premier**. L'historique du CRM a rempli le
**second**. Les deux existent : ce n'est pas une faute de frappe du retour, mais
un doublon fonctionnel côté org.

Le socle écrit désormais celui que le retour demande —
`LastMarketingEngagementDate__c`, derrière `@ECRIRE_DATE_ENGAGEMENT`. Alimenter
aussi le second est une **décision CRM** : c'est lui que lisent les rapports
existants, mais rien ne dit qu'ils doivent recevoir les engagements web.

> ⚠ Ne jamais choisir entre ces deux champs sur la foi du libellé. C'est le
> genre d'écart qui ne se voit pas en recette : un `<select>` mal trié se
> remarque, un champ date rempli au mauvais endroit ne se remarque jamais.

### Statut RGPD : le socle fait déjà sa part

`GDPR_Status__c` = `Marketing active` est écrit à la création depuis le 27/08,
dans **les deux** branches — vérifié dans le bloc DÉPLOYÉ, pas seulement dans
le dépôt. Si le champ ressort vide, la cause n'est pas dans le socle.

Ce que dit la donnée : **5 consentements sur 4227** portent une valeur
(3 `Marketing active`, 1 `To delete`, 1 `To reactivate`). Le champ est donc
quasi vide **sur toute l'org**, y compris sur les 4189 consentements créés par
le CRM lui-même. Deux hypothèses, à trancher côté CRM : valeur rejetée en
silence à l'insert (le connecteur ignore une valeur hors picklist sans
message), ou effacement par un flow après création. `ContactPointConsent` étant
CREATE-ONLY pour le connecteur, aucun rattrapage par update n'est possible.

### Marque : vide par décision, pas par oubli

`@ECRIRE_MARQUE_CONSENT` = `"false"` depuis le 27/08. Le champ était accepté le
matin, refusé l'après-midi, et ce refus **tuait la page à chaque soumission des
6 formulaires**. Rappel de la mesure : bissection `LPB_TST_Sonde_CPC`, les 8
autres champs passent, l'ajout du seul `BusinessBrandId` fait échouer.

Pour information, il est rempli sur **4189 / 4227** consentements de l'org : le
CRM y arrive, nous non. Avant de repasser le drapeau à `"true"`, refaire la
bissection — le rallumer à l'aveugle remet les 6 formulaires à l'arrêt.

### Sonde ajoutée

`A-COLLER-cloudpage-sonde-champs-cpc.ampscript` liste les noms d'API réels des
champs d'un objet (`?o=`, `?f=`, `?tout=1`). Lecture seule. C'est elle qui
tranche un nom de champ avant de l'écrire — règle du projet : on n'envoie que
des noms relevés sur l'org, parce qu'un nom faux ne rend pas une erreur
lisible, il tue la page.
