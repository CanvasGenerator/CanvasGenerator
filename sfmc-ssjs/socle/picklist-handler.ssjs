...
(function () {
    ...
    function bilanDe(html) {
        var m = /socle ecriture:\s*statut=(\w+)/i.exec(String(html || ''));
        if (!m) return { ok: false, message: '' };
        var e = /socle erreur:\s*([\s\S]*?)\s*-->/i.exec(String(html));
            ...
            window.fetch(window.location.href, {
              ...
              .then(function (html) {
                  ...
                  window.alert(bilan.message ||
                      "L'envoi n'a pas abouti. Merci de reessayer dans un instant.");
              })
              .catch(function () {
                  if (bouton) { bouton.disabled = false; bouton.innerHTML = libelle; }
