...
(function () {
    ...
    */
     ...
     *
     * Demande du 09/09 : « Full-Time » doit s'afficher « Initial » et
     * « Part-Time » « Alternance », pas « Temps plein » / « Temps partiel »
     * (les libelles CRM). La VALUE reste celle du CRM : c'est
    ...
    var SURCHARGES_LIBELLES = {
        rhythm: { 'Full-Time': 'Initial', 'Part-Time': 'Alternance' }
    };
    ...
    /* \u26a0 Accents en sequences \u, PAS en clair : ce fichier est recopie dans
       picklist-handler.ampscript puis televerse par l'API SFMC, ou un accent
       litteral se corrompt selon l'encodage. Le \u est du pur ASCII a la source
       et rend l'accent a l'execution. Meme regle que MESSAGES_BLOCAGE plus bas.
       Retour du 2026-09-03 : titre + corps de message, en remplacement du
       \u00ab une seule ligne \u00bb du 02/09. */
    var MESSAGES = {
        brochure: {
            titre: 'Votre brochure est pr\u00eate !',
            texte: 'Vous pouvez d\u00e8s maintenant la t\u00e9l\u00e9charger. '
                 + 'Elle vous a \u00e9galement \u00e9t\u00e9 envoy\u00e9e par email.'
        },
        candidature: {
            titre: 'Nous avons bien re\u00e7u votre demande de candidature',
            texte: 'Pour la finaliser et d\u00e9poser votre dossier, cr\u00e9ez '
                 + 'votre espace candidat via le lien envoy\u00e9 par e-mail '
                 + '(pensez \u00e0 v\u00e9rifier vos spams).'
        },
        evenement: {
            titre: 'Votre inscription est confirm\u00e9e !',
            texte: 'Merci pour votre inscription \u00e0 notre \u00e9v\u00e9nement. '
                 + 'Vous recevrez toutes les informations pratiques par email.'
        },
        immersion: {
            titre: 'Votre demande de participation est confirm\u00e9e !',
            texte: 'Notre \u00e9quipe des admissions vous contactera '
    ...
    /* ====================================================================
       ...
       Ce que le socle faisait jusqu'ici, et pourquoi c'etait faux. Sur un
       succes, `montrerSucces` masquait la zone de formulaire, les titres, et
       TOUS les enfants de la carte, puis ouvrait l'ecran `.xxx-success`
       centre. Le visiteur voyait un message seul au milieu d'une page vide :
       plus de formulaire, plus de valeurs, plus rien a relire. Le chemin du
       BLOCAGE, lui, faisait deja ce qu'il fallait — un encart au-dessus du
       bouton, formulaire intact — et c'est ce comportement qui est generalise
       ici a TOUTES les familles de formulaire, succes compris.

       D'ou l'encart unique. Confirmation, candidature deja en cours, decision
       defavorable et erreur de saisie partagent desormais le meme bloc, au
       meme endroit, et ne different que par le TON :

         succes  vert    la soumission a ete ecrite au CRM
         r1      orange  candidature deja en cours sur ce programme
         r2      rouge   decision defavorable rendue sur ce programme
         erreur  rouge   refus de saisie (numero de telephone, p. ex.)

       Un seul element plutot qu'un par etat, et c'est delibere : deux encarts
       distincts finiraient empiles le jour ou un visiteur enchaine un blocage
       puis un succes — il lirait « candidature en cours » sous « demande bien
       recue ». Un seul bloc, reecrit a chaque tentative, ne peut pas mentir.

       ⚠ L'ecran `.xxx-success` du HTML publie n'est plus jamais ouvert. Il
       reste dans la page, a son `display:none` de depart, et c'est ce qui rend
       ce retour applicable SANS republier les pages deja en ligne.
       ==================================================================== */
    var TONS = {
     ...
     *
     * `data-socle="message"` et non plus `"blocage"` : il ne porte plus
     * seulement des refus. Le nom ne vivait qu'a l'execution — aucune page
     * publiee ne le contient — donc le renommer ne casse rien en ligne.
     */
    function encartMessage(form) {
        var zone = form.querySelector('[data-socle="message"]');
        if (zone) return zone;
        ...
        if (zone.setAttribute) {
            ...
            /* `alert` et non `status` : qu'il annonce un refus ou une
               confirmation, l'encart est le SEUL retour de la soumission —
               plus rien ne bouge ailleurs dans la page. Un lecteur d'ecran
               doit l'entendre aussitot. */
        ...
        zone.style.borderLeftStyle = 'solid';
        zone.style.fontSize = '13px';
        zone.style.lineHeight = '1.5';
        zone.style.textAlign = 'left';

        /* Au-dessus du bouton plutot qu'a la fin du formulaire : le visiteur
     ...
     *
     * La PREMIERE ligne passe en gras des qu'il y en a plusieurs : c'est le
     * titre du message, et ce qui suit se lit comme son explication.
     */
    function montrerMessage(form, lignes, ton) {
        var zone = encartMessage(form);
        var couleurs = TONS[ton] || TONS.erreur;
        ...
        for (var i = 0; i < lignes.length; i++) {
            /* Une ligne `{cta:{...}}` devient un bouton, pas un paragraphe :
               c'est le « TELECHARGER LA BROCHURE » du retour du 03/09. */
            if (lignes[i] && lignes[i].cta) { zone.appendChild(boutonCta(lignes[i].cta)); continue; }
     ...
     *
     * Un <a> et non un <button> : c'est une navigation vers un fichier, et un
     * lien reste ouvrable dans un nouvel onglet, copiable, et lisible par un
     * lecteur d'ecran comme ce qu'il est.
     *
     * `target="_blank"` plutot que l'attribut `download` : le PDF est heberge
     * ailleurs que la CloudPage, et `download` est IGNORE en cross-origin — il
    ...
    function boutonCta(cta) {
        ...
        /* Le nouvel onglet n'est pose que sur une VRAIE URL — garde utile si
           la DE venait a porter une URL relative ou un simple `#`. */
        if (/^https?:/i.test(cta.href)) {
            a.setAttribute('target', '_blank');
            /* `noopener` : la page ouverte ne doit pas pouvoir manipuler
               celle-ci par `window.opener`. `noreferrer` en prime, l'URL de la
               CloudPage n'a rien a apprendre a l'hebergeur du PDF. */
     ...
     *
     * Rien n'est masque, et le formulaire garde ses valeurs : le visiteur
     * relit ce qu'il vient d'envoyer, et peut renvoyer — c'est exactement le
     * meme POST, le socle d'ecriture ne voit aucune difference. Sur une
     * candidature, changer de campus ou de rentree designe un autre PTAT :
    ...
    /* ====================================================================
       ...
       ⚠ CETTE LOGIQUE N'EST PAS TOUCHEE par le retour du 03/09. C'est le
       socle d'ECRITURE qui tranche, dans `handler-form.ampscript`, et il le
       fait correctement : cet ecran ne fait qu'afficher le motif recu.

       --- Le texte de R1 change, LUI, le 2026-09-03 --------------------
       Il annoncait « votre candidature a deja ete transmise [...] consultez
       l'email qui vous avait ete envoye » — une promesse d'email que rien ne
       garantit, et aucune porte de sortie. Le metier lui substitue le libelle
       que porte deja le socle d'ecriture : il ORIENTE le candidat vers les
       admissions du campus vise. Reformulation seule, a logique constante.
    ...
    function brancherSoumission(form) {
        ...
        form.addEventListener('submit', function (ev) {
            ...
            if (mauvaisTels.length) {
                ...
                montrerMessage(form, lignes, 'erreur');
            ...
            /* L'encart de la tentative precedente s'efface, quel qu'il ait
               ete. Un blocage porte sur un PROGRAMME et le candidat a pu en
               changer ; une confirmation porte sur un envoi qui n'est plus
               celui en cours. Dans les deux cas, le garder affiche pendant
               que le nouvel envoi part le ferait mentir. */
            effacerMessage(form);
            ...
            window.fetch(window.location.href, {
              ...
              .then(function (html) {
                  ...
                  /* ⚠ LE BOUTON EST RENDU DANS TOUS LES CAS, succes compris.
                     ...
                     RESTE, un bouton fige sur « Envoi en cours... » serait la
